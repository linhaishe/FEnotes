...
"导演": "克里斯托弗·诺兰",
"演员列表": [
    Actor(name="莱昂纳多·迪卡普里奥", role="柯布"),
    Actor(name="约瑟夫·高登-莱维特", role="亚瑟"),
    Actor(name="艾伦·佩吉", role="阿里阿德涅"),
    Actor(name="汤姆·哈迪", role="艾姆斯"),
...
```python
{'title': '盗梦空间', 'year': 2010, 'director': '克里斯托弗·诺兰'} # 必填，但无评分输出
# <class 'dict'>
```
...
```python
{'title': '盗梦空间', 'year': 2010, 'director': '克里斯托弗·诺兰', 'rating':0} # 必填，但有评分输出
# <class 'dict'>
```
...
在 Python 开发（特别是结合 LangChain / 大模型开发）中，选择 **`TypedDict`** 还是 **`Pydantic`** 主要取决于你是否需要 **运行时的“数据校验”与“功能扩展”**。

#### 一句话核心区别

- **`TypedDict`**：**轻量级静态类型提示**。本质上就是一个普通的 Python 字典（`dict`），仅在开发阶段（IDE/Mypy）做类型检查，**运行时没有任何校验**。
- **`Pydantic`**：**强类型运行时数据校验与解析库**。它不仅做类型检查，还会在运行时**强制校验**数据格式、转换类型、支持复杂的字段逻辑和方法扩展。

#### 详细对比与选型标准

| **特性 / 维度**  | **TypedDict**                       | **Pydantic (BaseModel)**                    |
| ---------------- | ----------------------------------- | ------------------------------------------- |
| **底层数据结构** | 标准 Python `dict`                  | 专门的 Pydantic 模型类                      |
| **运行时校验**   | **无**（传错类型不报错）            | **有**（类型不匹配或校验失败抛错）          |
| **类型自动转换** | **无**（给 `"123"` 就保持 `"123"`） | **有**（可自动将 `"123"` 转化为 `int 123`） |
| **性能开销**     | 零开销（与普通 dict 完全一致）      | 微小开销（涉及数据解析与校验逻辑）          |
| **方法与属性**   | 不能定义实例方法，仅存储数据        | 支持自定义方法、计算属性 (`@property`)      |
| **生态支持**     | 标准库（Python 3.8+）               | 外部库（FastAPI、LangChain 等重度依赖）     |

#### 场景选型建议

##### 场景 1：什么时候用 `TypedDict`？

当你的需求纯粹是**明确字典里有哪些 key、对应什么类型**，并且追求代码轻量、无外部依赖时使用。

- **内部函数间传递简单字典**：只需要给 IDE 智能提示，不需要开销。
- **LangGraph / LangChain 的 State 定义**：在定义 LangGraph 的图状态（Graph State）时，官方推荐优先使用 `TypedDict`，因为它本质上是一个普通的 dict，序列化和状态更新非常轻量。
- **只读的轻量配置或结构**：不需要校验外部输入，仅内部约定数据格式。
...
# 没有任何运行时校验，即便 age 传入字符串也不会报错
user: UserState = {"name": "Alice", "age": "18"}  
```

##### 2. 什么时候用 `Pydantic` (`BaseModel`)？

当你需要**处理外部不确定输入**、**定义 API/模型输出格式** 或 **实现复杂业务逻辑** 时使用。

- **结构化输出（Structured Output / Function Calling）**：向大模型请求 JSON/结构化输出时，强烈建议用 Pydantic，因为它可以把 Schema 完整地转为 OpenAI Tool/JSON Schema，并能对大模型返回的文本做格式校验。
- **FastAPI 请求/响应体定义**：API 接口的入参校验与自动化文档生成。
- **数据清洗与强校验**：自动做类型强转（如 `"2026-01-01"` -> `datetime`），或者需要复杂的正则、数值范围验证（`Field(gt=0)`）。
- **需要扩展数据方法**：除了存数据外，还需要在模型上定义辅助函数。
...
class UserModel(BaseModel):
    name: str
    age: int = Field(gt=0, description="必须是大于0的整数")

# 运行时校验：会自动将字符串 "18" 转为整数 18
user = UserModel(name="Alice", age="18") 

# 若传入非法数据（如 age=-5），运行时直接抛出 ValidationError 异常
```

#### 总结口诀

- **内部流转、状态传递、轻量提示** 用 **`TypedDict`**（简单、高效、无依赖）。
- **外部输入、大模型结构化输出、边界校验**  用 **`Pydantic`**（安全、强大、自带校验）。
...
```plaintext
               ┌── 需要存储数据？ ──┐
               │                   │
         [只用字典 dict]      [需要对象 class]
               │                   │
         ┌─────┴─────┐       ┌─────┴─────┐
         │           │       │           │
    [仅需类型提示] [需要校验] [内部可信数据] [外部不可信数据]
         │           │       │           │
         ▼           ▼       ▼           ▼
     TypedDict    Pydantic  @dataclass   Pydantic
```
...
1. **摆脱字典，拥有真正的类属性与方法** `TypedDict` 只能像 `obj["name"]` 这样用字典访问，而 `@dataclass` 生成的是真正的类，可以用 `obj.name`，并且可以在里面写自定义方法。
2. **标准库原生，无需安装任何第三方包** 如果你的代码/库不想依赖 Pydantic 这种庞大的外部依赖，但又想用优雅的面向对象（OOP）方式管理数据，`dataclass` 是首选。
3. **默认没有运行时校验** 与 `TypedDict` 一样，如果你给一个声明为 `int` 的字段传入了 `"abc"` 字符串，`dataclass` **不会**在运行时报错。

**适合使用 `@dataclass` 的场景：**

- **构建内部面向对象的数据模型**：当需要封装一些带有**行为/方法**的数据结构，且数据来源是**内部可信代码**（不需要防御式校验）时。
- **开发轻量级 SDK 或工具库**：不想为了简单的数据结构而引入 Pydantic 外部依赖。
- **追求内存优化/高性能对象**：配合 `@dataclass(slots=True)` 可以生成极具内存优势的 Python 对象。
