...
`TodoListMiddleware`中间件赋予了Agent 任务规划和追踪进度的能力，可以应对复杂的多步任务。
...
> 如果把普通的 Agent 比作“想到哪写到哪”的实习生，那么引入了 TodoListMiddleware 的 Agent 就是“先写方案、再列 CheckList、最后按部就班执行”的资深工程师。
...
2. `tool_description` —自定义`write_tools`工具的描述信息
...
扫描工作目录，测试并修复工作区下的`my_add.py`文件。
...
> 因此，可以把它理解成：主流程预留了一些插槽，允许你在这些位置挂上自己的函数，这种被挂进去并在特定时机执行的函数，就是 hook 函数。
...
- **Node-style（节点风格）** 采用的是 **“观察者 / 触发器模式”**（事件驱动）。
- **Wrap-style（包装风格）** 采用的是 **“装饰器 / 代理模式”**（中间件/洋葱模型）。

| **维度**       | **Node-style hooks (节点风格)**                              | **Wrap-style hooks (包装风格)**                              |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **设计模式**   | 事件回调 (Event Callbacks)                                   | 包装器/洋葱模型 (Decorator/Middleware Pattern)               |
| **执行控制权** | **无**（无法阻止下一个节点的执行，通常也无法直接拦截或替代原函数） | **有**（掌握原函数的执行权，决定**要不要调用**、**调用几次**） |
| **签名特征**   | `func(state, runtime) -> state`                              | `func(target_fn, *args, **kwargs)`                           |
| **典型应用**   | 记录日志、格式化输入/输出、发送审计通知、注入额外状态        | **错误重试（Retry）**、**缓存（Cache）**、**降级/回退（Fallback）**、**限流（Rate Limit）** |

以“**在模型调用前后记录耗时**”或“**修改模型输入的 Prompt**”为例：

- 用 `before_model` / `after_model` (Node-style) 可以做。
- 用 `wrap_model` (Wrap-style) 也可以做。

**只看/只修数据，不改变执行流程** -> **Node-style hooks**

- 例如：在模型调用前给 Prompt 拼接当前时间（`before_model`）；在模型调用后统计 Token 消耗（`after_model`）。

**要掌控执行流程（跳过、重试、捕获异常、替换实现）** ->  **Wrap-style hooks**

- 例如：如果 DeepSeek API 报错，自动切换成 OpenAI（Fallback 降级）；如果请求频繁，自动暂停 1 秒（Rate Limit）。

#### 类型1：Node-style hooks（节点风格钩子）

顾名思义，它们在流程的特定节点运行。适合顺序逻辑，如记录日志、验证
...
- 装饰器函数式挂载，把一个hook快速挂载到Agent的某个节点。
