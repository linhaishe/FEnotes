...
| 方式             | 代表方案                                   | 特点                                  |
| ---------------- | ------------------------------------------ | ------------------------------------- |
| 本地 Python 模型 | Sentence Transformers + Hugging Face       | 本地运行，适合学习和隐私数据          |
| 云端 API         | OpenAI、Cohere、Jina、Gemini 等            | 使用简单，效果稳定，但需要联网和付费  |
| 本地模型服务     | Ollama                                     | 通过本地 HTTP API 提供 Embedding      |
| 模型推理服务     | Hugging Face TEI、vLLM                     | 将 Embedding 模型部署成服务，适合生产 |
| LlamaIndex 集成  | `HuggingFaceEmbedding`、OpenAI Embeddings  | 直接接入 LlamaIndex RAG               |
| LangChain 集成   | `HuggingFaceEmbeddings`、OpenAI Embeddings | 直接接入 LangChain                    |
| 稀疏向量         | BM25、SPLADE                               | 更擅长关键词匹配，可与向量检索混合    |
...
查询也要使用同一个模型，不能文档用一个模型、问题用另一个模型后直接比较向量。
...
这样能分别理解本地 Embedding、云端 Embedding、混合检索和精排。

### Local embedding vs embedding api

有，主要缺点集中在**模型质量、部署成本和维护复杂度**上。

| 缺点                 | 具体表现                                                 |
| -------------------- | -------------------------------------------------------- |
| 模型质量可能不如 API | 本地模型的中文、英文、多语言和专业领域效果可能不稳定     |
| 需要计算资源         | 文档向量化和查询向量化都需要 CPU/GPU，数据量大时耗时明显 |
| 部署复杂             | 需要管理 Python 环境、模型文件、依赖、显存和服务进程     |
| 并发能力有限         | 多个用户同时检索时，本地推理服务可能成为瓶颈             |
| 模型文件较大         | 有些模型需要几百 MB 到几个 GB 的磁盘空间                 |
| 运维成本更高         | 需要自己处理模型加载、监控、扩容、重启和版本升级         |
| 模型更换成本高       | 换 Embedding 模型后，通常需要重新生成所有文档向量        |
| 结果调优需要经验     | 需要自己选择模型、向量维度、距离函数和批处理参数         |
| 长文本处理有限制     | 有些本地模型的最大输入长度较短，需要提前切分文本         |
| 更新不一定及时       | API 服务商通常会持续升级模型，本地模型需要自己更新       |
...
如果文档使用模型 A，查询使用模型 B，向量空间不一致，检索效果通常会明显下降。

此外，如果你已经使用模型 A 建好了 100 万条向量，后来换成模型 B：
...
| 场景                   | 更适合                     |
| ---------------------- | -------------------------- |
| 学习 RAG、数据量小     | 本地 Sentence Transformers |
| 快速做 Demo            | Embedding API              |
| 企业数据敏感           | 本地模型或私有化部署       |
| 调用量较小             | API                        |
| 调用量很大、长期运行   | 本地模型                   |
| 需要高质量多语言效果   | 先对比 API 和本地模型      |
| 需要快速上线           | API                        |
| 需要完全可控、离线运行 | 本地模型                   |
...
等流程跑通后，再把 Embedding 替换成 API，比较：
...
所以本地 Embedding 不是“效果一定差”，而是需要用更多工程成本换取隐私、可控性和长期成本优势。
