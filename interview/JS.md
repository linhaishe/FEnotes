...
>
> 变量提升不同：`var` 会提升并初始化为 `undefined`，`let` 和 `const` 存在暂时性死区；
>
> 重复声明：`var` 允许重复声明，`let` 和 `const` 不允许；
>
> 赋值规则：`const` 必须初始化且不能重新赋值；
>
> 全局对象：`var` 声明的变量会挂到 `window` 上，而 `let` 和 `const` 不会。
...
在 JavaScript 中，拷贝的差异主要体现在**引用类型**（Object, Array）上。

- **浅拷贝 (Shallow Copy)**：只拷贝对象的第一层属性。如果属性是基本类型，拷贝的是值；如果属性是引用类型，拷贝的是**内存地址**。因此，新旧对象会共享同一块子对象内存。
- **深拷贝 (Deep Copy)**：在堆内存中完整开辟一块新空间，递归地拷贝目标对象的所有层级。新旧对象完全隔离，互不影响。
...
- `Object.assign({}, obj)`
- 展开运算符 `[...arr]` 或 `{...obj}`
- `Array.prototype.slice()` 或 `concat()`
- 状态更新（如 React 的 `setState`）

深拷贝

- **简单方案**：`JSON.parse(JSON.stringify(obj))`
- **现代方案**：`structuredClone(obj)`（原生 Web API，2022年后主流浏览器均支持）
...
| **拷贝类型** | **应用场景**                                                 | **常见缺点**                                                 |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **浅拷贝**   | 状态更新（如 React 的 `setState`）、合并简单配置项、性能要求极高且确定无嵌套对象的场景。 | **数据污染**：修改新对象中的嵌套属性会意外改变原对象，导致难以追踪的 Bug。 |
| **深拷贝**   | 处理复杂的 JSON 响应、撤销重做功能（Undo/Redo）、彻底断开数据源关联的逻辑。 | **性能开销**：递归拷贝大对象耗时且占内存；**兼容性/局限性**：某些方式无法处理特殊类型。对象内部的循环引用会导致递归实现的深拷贝陷入死循环。 |

------

#### 2. 深度踩坑与解决方案（面试加分项）

在实现深拷贝时，最常见的三个“坑”及对策：

##### A. JSON 序列化的局限性

`JSON.stringify` 虽然简单，但有以下问题：

- **缺点**：会丢失 `Function`、`Symbol`、`undefined`；会将 `RegExp` 变为空对象，`Date` 变为字符串。
- **解决**：使用 `structuredClone()` 或手动编写递归函数。

##### B. 循环引用 (Circular Reference)

如果对象 A 引用了 B，B 又引用了 A，递归拷贝会进入死循环导致栈溢出。

- **解决**：使用 **WeakMap** 存储已拷贝过的对象。每次拷贝前先检查 Map 中是否存在，若存在直接返回，实现“引用的复用”。

| **类型 / 方案** | **JSON 方案** | **structuredClone (原生)** | **_.cloneDeep (Lodash)** | **手写递归 (基础版)** |
| --------------- | ------------- | -------------------------- | ------------------------ | --------------------- |
| **Date**        | 变成字符串 ❌  | 完美保留 ✅                 | 完美保留 ✅               | 变空对象 ❌            |
| **RegExp**      | 变空对象 ❌    | 完美保留 ✅                 | 完美保留 ✅               | 变空对象 ❌            |
| **Map / Set**   | 变空对象 ❌    | 完美保留 ✅                 | 完美保留 ✅               | 变空对象 ❌            |
| **Function**    | 丢失 ❌        | **抛出错误 ❌**             | 引用拷贝 ⚠️               | 丢失 ❌                |
| **循环引用**    | 报错 ❌        | 自动处理 ✅                 | 自动处理 ✅               | 栈溢出 ❌              |

##### C. 性能瓶颈

- **缺点**：对于极大的对象，深拷贝会造成主线程阻塞。
- **解决**：
  - **按需拷贝**：只拷贝变动的模块。
  - **Immutable 思想**：使用 Immutable.js 或 Immer.js。它们利用**持久化数据结构**和**结构共享**，在修改数据时只创建变化的部分，未变动部分继续共享，既保证了不可变性，又兼顾了性能。
...
> 针对原型、原型链 在定义上、使用场景上、缺点、优点输出面试回答
>
> 1. 定义：什么是原型与原型链？
>
> 在 JavaScript 中，原型机制是实现**继承**和**属性共享**的核心：
>
> - **原型 (Prototype)**：每一个函数在创建时，都会自动获得一个 `prototype` 属性，它是一个对象。这个对象就是由该函数创建的实例的“公共祖先”。
> - **原型链 (Prototype Chain)**：每个对象都有一个内部属性 `__proto__`（隐式原型/Dunder Proto）（即 `[[Prototype]]`），指向它的原型对象。当我们访问一个对象的属性时，如果对象自身没有，JS 引擎就会沿着 `__proto__` 向上查找，直到找到该属性或到达 `null`。这种链式结构就是原型链。
>
> ------
>
> 2. 使用场景
>
> 在实际开发中，原型主要解决**内存复用**和**逻辑继承**的问题：
>
> - **方法共享**：将通用的方法定义在构造函数的 `prototype` 上（如 `Array.prototype.push`），避免每个实例都创建一份方法的副本，节省内存。
> - **实现继承**：在 ES6 `class` 出现之前，通过原型链实现组合继承、寄生继承等模式。
> - **扩展内置对象**：例如给 `String.prototype` 添加一个自定义的格式化方法（虽然生产环境需谨慎，但这是原型的典型用法）。
> - **插件/库开发**：许多老牌库（如 jQuery）通过修改原型来扩展功能。
> 3. 优缺点
> | **维度**      | **优点**                                                     | **缺点**                                                     |
> | ------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
> | **性能/内存** | **内存利用率极高**。数万个实例可以共享同一个原型方法。       | **查找开销**。访问深层原型链上的属性比访问直接属性慢，且查找不存在的属性会遍历整条链。 |
> | **实时性**    | **实时性**。即便实例已经创建，修改原型，实例会立即获得新方法。 | **属性污染**。如果不小心修改了原型，所有实例都会受到影响。   |
> | **逻辑性**    | 实现了优雅的对象关联，比类继承更灵活。                       | **继承复杂性**。传统的原型继承代码冗余，容易在处理引用类型属性时出错。 |
>
> 4. 现代开发建议
>
> - **方案**：虽然底层是原型，但现代开发推荐使用 **ES6 `class`**。它本质是原型的语法糖，但提供了更清晰的语法，并解决了构造函数必须配合 `new` 使用等安全性问题。
>
> “是的，`class` 确实也要配合 `new` 使用。但我说它更安全，是因为它在引擎层面做了**强制校验**。
>
> 传统的构造函数如果不写 `new`，会静默失败并污染全局环境；而 `class` 会直接抛出错误。这就好比**‘手动挡’**和**‘自动挡’**的区别，`class` 帮我们装了防止误操作的‘电子限位器’。
>
> 此外，`class` 还通过**默认不可枚举方法**和**强制严格模式**，规避了许多原型链开发中常见的低级错误。”
...
>
> 常用于实现数据封装、状态保存和回调函数上下文管理。例如实现私有变量、回调异步、缓存函数结果等。但闭包会延长变量生命周期，如果引用了不必要的大对象可能导致内存占用增加，因此在使用时要避免滥用闭包、减少不必要的引用，并在不需要时及时释放引用。
>
> 合理使用闭包主要体现在三个方面：
>
> 1. 用闭包封装私有变量，避免全局污染；
...
可以按照“定义—背景—核心机制—API—优劣势—演进”的逻辑来组织

#### 1. 什么是 Promise？

**定义：** Promise 是 JS 中**异步编程**的一种解决方案，最早由社区提出，ES6 将其正式纳入标准。从语法上讲，它是一个**构造函数**；从功能上讲，它是一个**容器**，里面保存着某个未来才会结束的事件（通常是一个异步操作）的结果。

#### 2. 为什么要引入 Promise？（解决了什么问题）

在 Promise 出现之前，我们主要依靠**回调函数（Callback）**来处理异步。这会带来两个核心痛点：

1. **回调地狱 (Callback Hell)：** 当多个异步操作存在依赖关系时，代码会变成层层嵌套的“金字塔”结构。这不仅导致代码难以阅读，更让后期维护和错误追踪变得极其困难。
2. **控制反转 (Inversion of Control)：** 回调函数通常由第三方库执行，我们无法保证回调是否会被调用、调用次数是否正确、或者是否过早/过晚调用。Promise 则将异步操作的状态管理权收回到了开发者手中。

------

#### 3. Promise 的基本概念：三个状态与两个阶段

Promise 实例具有**三个状态**，且状态的改变是**不可逆**的：

- **Pending（进行中）**：初始状态，既没有被兑现，也没有被拒绝。
- **Fulfilled（已成功）**：意味着操作成功完成。
- **Rejected（已失败）**：意味着操作失败。

**状态改变规则：** 状态只能从 `Pending -> Fulfilled` 或者从 `Pending -> Rejected`。一旦状态凝固（Resolved），就不会再变。

------

#### 4. 常用的 API

Promise 提供了丰富的原型方法和静态方法：

- **原型方法：**
  - `.then()`: 注册状态变为 Fulfilled 后的回调。
  - `.catch()`: 捕获异常，处理状态变为 Rejected 后的逻辑。
  - `.finally()`: 无论状态如何，最终都会执行（如关闭 Loading 状态）。
- **静态方法（并发控制）：**
  - `Promise.all()`: 接收多个 Promise，全部成功才成功，有一个失败就失败。
  - `Promise.race()`: 赛跑机制，谁先返回结果（无论成败）就用谁。
  - `Promise.allSettled()`: 等待所有 Promise 结束，无论成功还是失败，并返回每个任务的状态数组。
  - `Promise.any()`: 只要有一个成功就返回该成功的 Promise，全部失败则报错。
...
1. **链式调用（Chaining）：** 将异步操作以同步的流程表达出来，避免了嵌套。
2. **错误穿透：** 通过 `.catch()` 可以统一处理整个链条上的错误。
3. **状态保护：** 一旦确立，外界无法改变其内部状态，保证了结果的唯一性。

##### 缺点：

1. **无法取消：** 一旦新建 Promise 就会立即执行，无法中途取消。
2. **错误屏蔽：** 如果不设置回调函数（没有 `.catch()`），内部抛出的错误不会反应到外部。
3. **状态观测难：** 处于 Pending 状态时，无法得知当前进展到了哪一步（是刚开始还是快结束了）。

------

#### 6. 进阶：从 Promise 到 Async/Await

虽然 Promise 解决了回调地狱，但它依然包含大量的 `.then()` 模板代码，且逻辑较多时依然不够直观。

为了追求**“用同步的方式写异步代码”**，ES8 引入了 `async/await`。

- **async/await 是 Promise 的语法糖**，它底层基于 Generator 函数和自动执行器。
- 它让异步代码在视觉上和同步代码几乎无异，彻底消灭了链式调用，使得 try-catch 捕获错误也更加自然。
