...
JSX 的本质是 React.createElement 的语法糖，本质上还是 JavaScript。JSX 会在编译阶段（Babel）被转换成普通的 JS 函数调用。
...
这是因为 JSX 实际上会被编译为 React.createElement(...) 的形式。即使你在代码中没有直接使用 React，但编译生成的代码依赖它，因此必须确保 React 被导入。如果使用 React 17 或以上的版本，且 Babel 配置开启了新 JSX 转换机制，则可以不导入 React。
...
Fiber 是 React16 引入的一种新的数据结构，表示组件树中每个节点的“工作单元”。它用 `child`、`sibling`、`return` 指针组成树状链表，保存组件类型、状态和属性等信息，支持可中断、可恢复的异步增量渲染，实现任务优先级调度，提高渲染效率。

Fiber 本质上是一种链表结构，每个 Fiber 对象对应 React 虚拟 DOM 树中的一个节点，它包含了当前节点的状态、属性、子节点关系等关键信息。
