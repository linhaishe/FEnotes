...
JavaScript 的事件模型核心是 **事件流**（捕获 → 目标 → 冒泡）

事件绑定可以选择**捕获阶段**或**冒泡阶段**

可以通过 `preventDefault` 和 `stopPropagation` 控制事件行为

DOM2 级事件绑定最推荐，支持多处理函数和事件流控制

### 3. ==Javascript本地存储的方式有哪些？区别及应用场景？==localstorage
