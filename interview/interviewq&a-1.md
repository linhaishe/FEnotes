...
`setTimeout` 属于宏任务，`Promise.then` 属于微任务，`async/await` 是基于 Promise 的语法糖，本质仍然是微任务。
...
所谓Promise，简单说就是一个容器，里面保存着某个未来才会结束的事件（通常是一个异步操作）的结果。从语法上说，Promise 是一个对象，它可以获取异步操作的消息。Promise 提供统一的 API，各种异步操作都可以用同样的方法进行处理。特点是：状态不可逆，支持链式调用，解决回调地狱
...
async/await其实是  Generator + Promise + 自动执行器 的语法糖，它能实现的效果都能用then链来实现，它是为优化then链而开发出来的。从字面上来看，async是“异步”的简写，await则为等待，所以很好理解async 用于申明一个 function 是异步的，而 await 用于等待一个异步方法执行完成。当然语法上强制规定await只能出现在asnyc函数中。
...
> **回调函数（Callback）是作为参数传入另一个函数，并在某个时机被调用的函数。**
...
- [检验HTTP Referer](https://www.cnblogs.com/mysticbinary/p/12630011.html#检验http-referer)
- [验证码](https://www.cnblogs.com/mysticbinary/p/12630011.html#验证码)
- Token
  - [Cookie Token](https://www.cnblogs.com/mysticbinary/p/12630011.html#cookie-token)
  - [HTTP头自定义属性Token](https://www.cnblogs.com/mysticbinary/p/12630011.html#http头自定义属性token)
  - [表单Token](https://www.cnblogs.com/mysticbinary/p/12630011.html#表单token)

但是使用验证码是完全可以做到防止CSRF的，因为验证码是用户在提交表单的时候，必须输入图片验证码，保证了服务器收到的是来自预期的请求
...
- tree层级
  ...
  <img src="http://tva1.sinaimg.cn/large/005NUwygly1h8cwwb66ujj30gp08kq5n.jpg" alt="image.png" style="zoom: 50%;" /><img src="http://tva1.sinaimg.cn/large/005NUwygly1h8cwx3fwl4j30k80asq4d.jpg" alt="image.png" style="zoom: 50%;" />

- component 层级

  如果是同一个类的组件，则会继续往下diff运算，如果不是一个类的组件，那么直接删除这个组件下的所有子节点，创建新的。当`component D`换成了`component G` 后，即使两者的结构非常类似，也会将`D`删除再重新创建`G`

  <img src="http://tva1.sinaimg.cn/large/005NUwygly1h8cwxso3o2j30jz08ggnl.jpg" alt="image.png" style="zoom:50%;" />

- element 层级

  对于比较同一层级的节点们，每个节点在对应的层级用唯一的`key`作为标识，提供了 3 种节点操作，分别为 `INSERT_MARKUP`(插入)、`MOVE_EXISTING` (移动)和 `REMOVE_NODE` (删除)

  <img src="http://tva1.sinaimg.cn/large/005NUwygly1h8cwy29sjbj30p30eygnw.jpg" alt="image.png" style="zoom:50%;" />
...
- 字符串格式：字符串格式，例如：`<p ref="info">span</p>`
- 函数格式：ref对应一个方法，该方法有一个参数，也就是对应的节点实例，例如：`<p ref={ele => this.info = ele}></p>`
- createRef方法：hooks，使用React.createRef()来实现
- 函数组件：`const textInput = useRef(null);`
...
<img src="http://tva1.sinaimg.cn/large/005NUwygly1h8bxir4dn8j30k10kk41o.jpg" alt="image.png" style="zoom: 50%;" />
...
### 12. react 18 版本中 setState 是同步还是异步的？
...
### 22. react ref 是如何拿到函数组件的实例

### 23. react useRef、ref、forwardsRef 的区别是什么?

### 24. react forwardRef 作用是什么？

### 25. react forwardsRef 作用是啥， 有哪些使用场景？
...
<img src="http://tva1.sinaimg.cn/large/005NUwygly1h8cqwlbprgj30ly0hc40z.jpg" alt="image.png" style="zoom: 50%;" />
...
- 在组件生命周期或React合成事件中，setState是异步，因为react可以控制。
...
setState 的第二个参数是一个可选的回调函数。这个回调函数将在组件重新渲染后执行。等价于`componentDidUpdate` 生命周期内执行。通常建议使用 `componentDidUpdate `来代替此方式。在这个回调函数中你可以拿到更新后 state 的值
...
<img src="http://tva1.sinaimg.cn/large/005NUwygly1h8cwbqdjdpj30lr0dqmzz.jpg" alt="image.png" style="zoom:50%;" />
...
`shouldComponentUpdate`提供了两个参数`nextProps`和`nextState`，表示下一次props和下一次state的值，当函数返回false时候，`render()`方法不执行，组件也就不会渲染，返回true时，组件照常重渲染。此方法就是拿当前props中值和下一次props中的值进行对比，数据相等时，返回false，反之返回true。
...
<img src="http://tva1.sinaimg.cn/large/005NUwygly1h8ctlvxig9j30so0zw0xh.jpg" alt="image.png" style="zoom:33%;" /><img src="http://tva1.sinaimg.cn/large/005NUwygly1h8cto34k6fj30oc12kdkn.jpg" alt="image.png" style="zoom:33%;" />
...
### 5. react hooks 调用是可以写在 if 语句里面吗

### 6. react 为什么不能在循环、条件或嵌套函数中调用 Hooks？

### 7. react useRef 是如何实现的

`useRef` 接受一个初始值，返回一个包含 `.current` 属性的可变对象，该对象在整个组件生命周期内保持不变。

React 内部通过 Fiber 节点上的 hook 链表存储和管理 `useRef` 的状态，首次渲染时创建对象，后续渲染复用同一对象。与 `useState` 不同，修改 `.current` 不会触发组件更新，适合保存跨渲染周期的持久数据或引用 DOM 节点。

```js
function useRef(initialValue) {
  const hook = getCurrentFiberHook();

  if (!hook) { // 首次渲染，创建 hook
    const refObject = { current: initialValue };
    registerHook({
      memoizedState: refObject,
    });
    return refObject;
  } else { // 更新渲染，复用之前的 hook
    return hook.memoizedState;
  }
}
```

- 当组件首次执行时，`useRef` 会在当前 Fiber 节点的 hook 链表中创建一个新的 hook 对象，其内容形如：
...
- `memoizedState` 用于保存 hook 的状态数据，`useRef` 返回的对象就是这个 `memoizedState`。
- 后续渲染时，React 会复用该 hook 节点，直接返回之前存储的 `memoizedState`，保证了引用对象的恒定性。

### 8. react ref 有哪些使用场景，请举例
...
### 18. react useEffect 依赖为空数组与 componentDidMount 区别
...
### 19. react useLayoutEffect 和 useEffect 有什么区别?
...
### 20. react useEffect的第二个参数，如何判断依赖是否发生变化
...
<img src="http://tva1.sinaimg.cn/large/005NUwygly1h7l0c2qyivj30i80hs41j.jpg" alt="image-20210906154354133.png" style="zoom: 50%;" /><img src="http://tva1.sinaimg.cn/large/005NUwygly1h7l0ca7p6vj30k809e75v.jpg" alt="image-20210906154407089.png" style="zoom: 50%;" />
