...
- **defer 属性：给 `<script>` 标签添加 `defer` 属性后，浏览器在解析 HTML 时会 **并行（异步）下载脚本文件，而不会阻塞 HTML 的解析和页面渲染**。等到 **整个文档解析完成（DOM 构建完成）后**，再按照 **脚本在文档中的顺序依次执行**这些脚本。

  使用 `defer` 可以避免 JavaScript 阻塞页面渲染，从而提高页面加载性能。多个带有 `defer` 的脚本通常会 **按照在 HTML 中出现的顺序执行**。

- **async 属性：`async` 会让脚本 **异步下载**，下载完成后 **立即执行**。执行时会 **暂停 HTML 解析**。多个 `async` 脚本之间 **不保证执行顺序**。
...
> **普通 script = 同步下载 + 立即执行 + 阻塞 HTML 解析**

| 特性             | script   | defer         | async            |
| ---------------- | -------- | ------------- | ---------------- |
| 下载             | 同步     | 异步          | 异步             |
| 是否阻塞HTML解析 | 会       | 不会          | 下载不阻塞       |
| 执行时机         | 立即执行 | DOM解析完成后 | 下载完成立即执行 |
| 执行顺序         | 按顺序   | 按顺序        | 不保证顺序       |

### ==3. Javascript本地存储的方式有哪些？区别及应用场景？==

cookie、sessionStorage、localStorage 和 IndexedDB 都是浏览器提供的客户端存储方案。
 cookie 主要用于客户端和服务器之间通信，每次 HTTP 请求都会自动携带，容量约 4KB。
 localStorage 和 sessionStorage 属于 Web Storage，容量约 5MB，只能存字符串，其中 localStorage 是永久存储，而 sessionStorage 是会话级存储。
 IndexedDB 是浏览器提供的本地数据库，支持结构化数据存储，容量更大，并且是异步 API。

| 特性                 | cookie            | sessionStorage     | localStorage             | IndexedDB              |
| -------------------- | ----------------- | ------------------ | ------------------------ | ---------------------- |
| 存储位置             | 浏览器            | 浏览器             | 浏览器                   | 浏览器                 |
| 生命周期             | 可设置过期时间    | 页面会话关闭即销毁 | 永久存储（除非手动清除） | 永久存储               |
| 存储大小             | ~4KB              | ~5MB               | ~5MB                     | 几百 MB 甚至更多       |
| 是否随 HTTP 请求发送 | **会自动发送**    | 不会               | 不会                     | 不会                   |
| 访问方式             | `document.cookie` | Web Storage API    | Web Storage API          | IndexedDB API          |
| 数据类型             | 字符串            | 字符串             | 字符串                   | **支持结构化数据**     |
| 是否同源限制         | 是                | 是                 | 是                       | 是                     |
| 使用复杂度           | 简单              | 简单               | 简单                     | **复杂（数据库 API）** |
...
**localStorage 本身不支持过期时间机制**，因此如果需要实现数据过期功能，需要在存储数据时 **额外保存一个过期时间戳**，在读取数据时 **判断当前时间是否超过该时间戳，如果已过期则删除对应数据并返回空值**。

> `localStorage` 是 **同步 API**，大量读写可能会阻塞主线程，因此不适合存储大量或频繁更新的数据。
...
```js
setStorage("token", "123456", 60 * 1000); // 1分钟过期
getStorage("token");
```
...
JSBridge 是一种 Web 与 Native 之间的通信机制，常用于 Hybrid App 开发。由于运行在 WebView 中的 JavaScript 无法直接调用 Native API，因此需要通过 JSBridge 建立通信桥梁。

其工作原理是通过 WebView 提供的接口实现消息传递，例如通过 URL Scheme 拦截、JavaScript Interface 或 postMessage 等方式让 JS 调用 Native 方法，同时 Native 也可以通过执行 JavaScript 代码回调 H5 页面，从而实现双向通信。

它主要用于 **混合开发（Hybrid App）** 场景，让 **Web 页面中的 JavaScript 可以调用 Native 能力，同时 Native 也可以调用 Web 的 JavaScript 方法**。JSBridge = Web 与 Native 之间的通信桥梁

**WebView** 是一个 **嵌入在原生应用（Android / iOS）中的浏览器组件**，可以在 App 内部加载和显示 **网页内容（HTML / CSS / JavaScript）**。

简单理解：

> **WebView = App 里的一个小浏览器**
...
> **利用 WebView 提供的能力，实现 JS 和 Native 之间的消息传递。**
...
> 1. DNS 解析获取服务器 IP（**DNS 就是互联网的“电话簿”，把域名翻译成 IP 地址，让浏览器找到服务器。**
> 2. TCP 三次握手建立连接，HTTPS 还会做 TLS 握手
> 3. 浏览器发送 HTTP 请求并接收响应
> 4. 浏览器解析 HTML、CSS、JS，生成 DOM、CSSOM 和渲染树
> 5. 计算布局（Layout / Reflow）并绘制（Paint / Composite）到屏幕
> 6. JS 引擎执行脚本，操作 DOM，可能触发重排重绘
>    浏览器内部主要有 **网络层、渲染引擎、JS 引擎、UI 后端和数据存储** 等模块，浏览器加载页面是一个 **边接收边解析、边渲染边执行 JS 的流水线过程**。由于 JS 可能修改 DOM，因此 **普通 script 会阻塞 HTML 解析**，这也是为什么需要 `defer` 和 `async` 优化加载。
...
==减少阻塞、减少重排重绘、优化渲染路径、合理加载资源（**图片与资源优化**、**缓存优化**）==

1.**减少渲染阻塞**：JS 放底部、使用 defer/async，关键 CSS 内联，减少 @import；

2.**减少重排重绘**：合并 DOM 操作、使用 transform/opacity 做动画、避免频繁访问 layout 属性；

> 重排（Reflow）是指元素几何属性变化时，浏览器重新计算布局；重绘（Repaint）是指样式变化但布局不变时重新绘制像素。
>  如果不优化，频繁触发重排和重绘会导致页面卡顿、掉帧、首屏渲染慢、滚动和动画不流畅，尤其在移动端性能影响明显。优化策略包括合并 DOM 操作、使用 transform/opacity 动画、避免频繁读取 layout 属性和减少复杂布局。

3.**优化关键渲染路径**：优先渲染 Above-the-Fold(首屏) 内容，拆分 JS/CSS；

> A. **减少关键渲染路径长度**
>
> 1. 优先加载 **Above-the-Fold（首屏内容） 内容**
>
> ​	将首屏 CSS **内联**到 HTML `<head>`，保证首屏样式早解析
>
> 2. 推迟加载非首屏 CSS（懒加载、按需加载）
>
> 3. 首屏图片使用小图或占位图，懒加载剩余图片
>
> 4. 合理拆分 JS/CSS，做到 **首屏快速渲染**
>
> - **大文件 JS / CSS 会阻塞渲染**：
>   - 普通 `<script>` 会阻塞 HTML 解析
>   - 大 CSS 文件会阻塞浏览器生成渲染树
> - 优化策略：
>   - **按路由拆分 JS**（Code Splitting / 按需加载）
>   - **非首屏 CSS 延迟加载**（使用 `media` 属性或 JS 动态加载）
>   - 对 JS 使用 `defer` / `async`
>
> **效果**：浏览器可以先渲染首屏内容，后续 JS / CSS 再加载，减少用户等待。
>
> B. 使用 **浏览器缓存 / CDN** 加快静态资源加载
>
> **缓存**：
>
...
> - 提高首屏渲染速度
>
>  一、什么是“关键渲染路径”（Critical Rendering Path）
>
> 浏览器从输入 URL 到页面可见，需要经历 **一系列资源加载和解析**，称为 **关键渲染路径**：
>
> 1. **HTML** → 解析成 **DOM 树**
> 2. **CSS** → 解析成 **CSSOM 树**
> 3. **DOM + CSSOM** → 构建 **渲染树（Render Tree）**
> 4. **布局（Layout）** → 计算每个元素位置
> 5. **绘制（Paint）** → 渲染像素
>
> > **关键渲染路径**就是让浏览器把 **首屏可见内容** 显示出来所必须经过的资源和步骤。
>
> 二、为什么要优化关键渲染路径
>
> - 首屏加载慢 → 用户感知页面卡顿
> - JS / CSS 阻塞解析 → 延迟 DOM 渲染
> - 首屏渲染速度慢 → 用户体验差
>
> 优化目标：**尽快显示 Above-the-Fold 内容（首屏内容）**，让用户看到页面不空白。

4.**图片与资源优化**：压缩、懒加载、WebP、字体优化

5.**缓存优化**：HTTP 缓存、Service Worker、IndexedDB；

> 如果缓存优化不到位，每次访问页面都要重复下载资源，导致页面加载变慢、首屏渲染延迟，同时增加服务器压力和带宽消耗。在移动端或弱网环境下，用户体验会显著下降；对于 PWA，离线访问也无法正常工作。
>
> **重复网络请求，页面加载变慢**
>
> - 每次访问页面或刷新都要重新下载静态资源（JS、CSS、图片）
> - 导致 **首屏渲染慢**，用户感知性能差
>
> **带宽浪费，增加服务器压力**
>
...
> **离线或弱网体验差**
>
> - PWA 或离线应用依赖 Service Worker / IndexedDB 缓存
> - 如果没缓存，用户断网时无法访问页面
>
> **页面闪烁 / FOUC（Flash of Unstyled Content）**
>
> - CSS 或字体没有缓存时，可能闪现默认样式

> 浏览器缓存优化主要通过减少重复请求和加快资源获取来提升页面性能。常见方法包括：
>
> 1. 合理使用 HTTP 缓存，包括强缓存（Cache-Control、Expires）和协商缓存（ETag、Last-Modified）；
> 2. 静态资源使用 hash 或版本号进行缓存更新控制；
> 3. 使用 CDN 提高资源分发速度；
> 4. 通过 Service Worker 实现离线缓存；
> 5. 使用 localStorage 或 IndexedDB 缓存部分数据。
>
> 通过这些策略可以减少网络请求，提高首屏加载速度。

6.**利用 GPU 合成层**：提升动画性能，减少重排重绘开销。

> 1. **动画卡顿 / 不流畅**
>    - 浏览器 CPU 进行大量布局计算和绘制
>    - FPS 下降，出现掉帧
> 2. **频繁重排（Reflow）和重绘（Repaint）**
>    - 对 DOM 元素操作导致整个渲染树重新计算布局
>    - 页面性能严重下降
> 3. **滚动或拖拽体验差**
>    - 大量 repaint 导致界面响应慢
>    - 用户感知“卡顿”
...
>    - 页面复杂动画不优化，滑动和动画都会掉帧
>
> > 如果不利用 GPU 合成层，动画和滚动等操作都会在 CPU 上完成，频繁触发重排和重绘，导致页面掉帧、卡顿和不流畅，尤其在移动端和复杂页面上用户体验明显下降。
>
> 哪些 CSS 会触发 GPU 层合成
>
> 1. **transform**
...
> 2. **opacity**
> 3. **will-change**
>    - 告诉浏览器 “这个元素即将变化”，提前创建 GPU 层
> 4. **filter / perspective / 3D transform**（部分情况）
>
> **不推荐的属性**：`top`, `left`, `width`, `height`, `margin` 等
>
> - 修改这些属性会触发 **layout + repaint**
> - GPU 层不会帮你避免重排

利用 GPU 合成层：提升动画性能，减少重排重绘开销。这个难道不属于减少重排重绘那部分的吗

| 优化点              | 核心目标                                | 实现方式                                                     | 和重排重绘的关系                                             |
| ------------------- | --------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **减少重排重绘**    | 避免 CPU 频繁计算布局和像素绘制         | 合并 DOM 操作、减少访问 layout 属性、用 transform/opacity 代替 top/left | 从 CPU 角度减少渲染开销，是通用优化                          |
| **利用 GPU 合成层** | 把动画或可视元素交给 GPU 渲染，提高帧率 | transform / opacity / will-change 等触发 GPU 层              | 可以 **进一步减少重排重绘开销**，特别是动画/滚动场景，属于“高级优化” |

浏览器渲染优化主要针对减少重排（Reflow）、减少重绘（Repaint）、优化 JS 加载和网络资源、合理使用分层渲染和 GPU 加速。重排代价最大，应该避免批量操作 DOM、避免频繁读取布局属性。动画推荐使用 transform 和 opacity，减少阻塞渲染的同步 JS 和阻塞 CSS，图片懒加载和压缩可以提升性能。
...
进程是资源分配的最小单位，线程是 CPU 调度的最小单位。进程拥有独立资源，一个进程可以包含多个线程，线程共享进程资源，线程切换开销更小，但稳定性较差。
...
Service Worker 运行在浏览器和网络之间，通过 install 阶段缓存资源，在 fetch 阶段拦截请求并根据缓存策略返回缓存或网络数据，当网络不可用时直接从 Cache Storage 返回响应，从而实现离线访问。常见策略包括 Cache First、Network First 和 Stale While Revalidate。

> 使用 `install` 事件 + `Cache API` 缓存文件，`fetch` 事件拦截请求返回缓存

> **Service Worker 通过拦截网络请求，把静态资源或接口数据缓存到 Cache Storage，在离线或弱网时直接从缓存返回，实现离线访问。**

### Q1：SW 为什么能离线？

> 因为 SW 运行在浏览器与网络之间，能拦截请求并从 Cache Storage 直接返回响应。
...
> HTTP 缓存由浏览器控制，SW 缓存由开发者完全控制，策略更灵活。

# 3. 浏览器缓存

## ==1. 对浏览器的缓存机制的理解==

**浏览器缓存**是指浏览器在访问网页时，将静态资源（HTML、CSS、JS、图片等）存储在本地，以便下次访问同一资源时可以 **直接使用缓存而不是再次向服务器请求**，从而提高页面加载速度、减少网络请求、降低服务器压力。

> **缓存 = 浏览器的“临时仓库”，用来存储已经下载过的资源，加快页面加载。**
...
- 浏览器直接使用缓存，不会向服务器发送请求
- 常用 HTTP 头：
  - `Cache-Control: max-age=3600` → 缓存一小时
  - `Expires: Sat, 15 Mar 2026 16:00:00 GMT` → 过期时间
- 特点：**不与服务器交互**，效率最高

1. **协商缓存（Conditional Request）**

- 浏览器先使用缓存，但会向服务器询问资源是否更新
- 常用 HTTP 头：
  - `Last-Modified` + `If-Modified-Since` → 服务器返回 304 表示未修改
  - `ETag` + `If-None-Match` → 内容指纹，未修改返回 304
- 特点：**减少下载，仍需要一次请求**

1. **其他缓存机制**

- **Service Worker Cache**：PWA 离线缓存
- **IndexedDB / localStorage**：前端存储数据，用于减少 API 请求或存储应用状态
- **Memory Cache**：浏览器内存缓存，页面刷新后失效
- **HTTP/2 Push Cache**：提前推送资源到浏览器缓存
...
> 浏览器事件机制基于 **事件流（捕获 → 目标 → 冒泡）** 处理事件，开发者通过 **事件监听** 和 **事件委托** 控制事件触发和传播，从而高效管理用户交互和 DOM 操作。+ 解释事件流 + 解释事件委托
>
> 1. **事件流（Event Flow）**
>     浏览器事件流分为三个阶段：
>
> - **捕获阶段（Capture）**：事件从根节点 `window` 向目标节点传播
> - **目标阶段（Target）**：事件到达目标元素
> - **冒泡阶段（Bubble）**：事件从目标元素回到根节点
>    默认情况下，大部分事件都是冒泡触发，但可以通过 `addEventListener(type, handler, true)` 显式绑定捕获阶段。
>
> 1. **事件机制（Event Mechanism）**
>
> - 开发者可以通过 `addEventListener` 或 DOM0 `onclick` 等方式绑定事件
> - 事件对象 `event` 提供事件类型、目标元素、键盘/鼠标信息等
> - 常用方法：
>   - `event.preventDefault()` 阻止默认行为
>   - `event.stopPropagation()` 阻止冒泡
> - 事件机制依赖事件流，浏览器根据捕获/目标/冒泡顺序触发绑定的处理函数
>
> 1. **事件委托（Event Delegation）**
>
>    > 事件委托利用冒泡把事件绑定到父元素上，从而减少绑定数量、兼容动态元素，结合事件对象可以灵活控制行为。
>
> - 利用事件冒泡，把子元素的事件绑定在父元素上
> - 优势：
>   1. **减少事件绑定** → 节省内存和 CPU
>   2. **动态元素兼容** → 后加入的子元素也能触发事件

## 1. ==事件是什么?事件模型/事件流模型?==

浏览器事件机制基于事件流（捕获 → 目标 → 冒泡），支持事件监听和事件委托，通过阻止冒泡或默认行为可以灵活控制事件传播，从而高效处理用户交互。

浏览器事件机制主要包括**事件捕获、事件冒泡和事件委托**，它是浏览器处理用户交互和 DOM 变化的重要机制。
...
可能会引申到什么是闭包/如何合理使用闭包

> JavaScript 的垃圾回收机制主要用于自动释放不再使用的内存，防止内存泄漏。现代浏览器主要采用**可达性分析**，从根对象出发，回收不可达对象；早期也有**引用计数**机制，但无法处理循环引用。常见内存泄漏场景包括全局变量、闭包滥用、未清理的定时器或事件监听、DOM 引用未释放等。优化手段包括减少全局变量、及时清理事件监听和定时器、合理使用 WeakMap/WeakSet 和闭包。

> 函数在创建时会形成一个词法作用域，函数内部的函数引用了外部函数的变量，即使外部函数已经执行完毕，这些变量也不会被销毁，而是被内部函数“闭合”保存起来。
>
> 闭包可以让函数访问外部作用域变量，常用于实现数据封装、状态保存和回调函数上下文管理。例如实现私有变量、缓存函数结果等。但闭包会延长变量生命周期，如果引用了不必要的大对象可能导致内存占用增加，因此在使用时要避免滥用闭包、减少不必要的引用，并在不需要时及时释放引用。
>
> 合理使用闭包主要体现在三个方面：
>
> 1. 用闭包封装私有变量，避免全局污染；
> 2. 在需要保存状态或缓存数据的场景使用闭包；
> 3. 避免不必要的闭包引用大对象，并在不使用时及时释放引用，防止内存占用增加。

浏览器垃圾回收机制是指 JavaScript 引擎自动回收不再使用的内存，防止内存泄漏和页面性能下降。它主要依赖**引用计数**和**可达性分析（Reachability / Mark-and-Sweep）**两种策略。

浏览器垃圾回收的核心思想是：回收不再被引用的内存，避免内存泄漏。
现代浏览器主要采用“标记清除”为主的垃圾回收算法，并结合分代回收等优化策略。
